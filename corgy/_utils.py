...
def _check_val_type(_val, _type, try_cast=False):
    ...
    if _coll_type is not None:
        ...
        if len(_base_types) == 1:
            ...
            for _val_i in _val:
                _cast_val_is.append(_check_val_type(_val_i, _base_types[0], try_cast))
            ...
            for _val_i, _base_type_i in zip(_val, _base_types):
                _cast_val_is.append(_check_val_type(_val_i, _base_type_i, try_cast))

        _val = _cast_type(_cast_val_is)
        return _val

    if _is_optional_type(_type):
        if _val is None:
            return None
        _base_type = _type.__args__[0]
        return _check_val_type(_val, _base_type, try_cast)

    if _is_literal_type(_type):
